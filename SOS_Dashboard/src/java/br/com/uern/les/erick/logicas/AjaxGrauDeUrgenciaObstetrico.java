 ...
 * @author jerick.gs
 */
public class AjaxGrauDeUrgenciaObstetrico implements LogicaAjax{
    ...
    public String executaAjax(HttpServletRequest req, HttpServletResponse res) throws Exception {
        ...
        float perdaLiquido = Float.parseFloat(req.getParameter("perdaLiquido"));
        float dorIntensidade = Float.parseFloat(req.getParameter("dorIntensidade"));
        float perdaSangue = Float.parseFloat(req.getParameter("perdaSangue"));
        float preNatal = Float.parseFloat(req.getParameter("preNatal"));
        ...
        float GSf = (perdaLiquido + dorIntensidade + perdaSangue) / 3;
        
        //CALCULO GRAVIDADE ETIOLOGICA (MÉDICO)
        float GEf = (gravidadeCaso + preNatal) / 2;
